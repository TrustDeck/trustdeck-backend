...
public class PseudonymKafkaListener {
	...
	/** Provides access to default values. */
    @Autowired
    private DefaultProperties defaults;

    /** Connects the pseudonym controller where the actual processing is done. */
    @Autowired
    private PseudonymController pseudonymController;
	...
	public void onMessage(Message<String> message, Acknowledgment ack) {
        ...
        if (domain == null) {
            ...
            d.setValidto(Utility.plusValidityTime(d.getValidfrom(), defaults.getDomain().getValidityTime()));
            d.setValidtoinherited(false);
            d.setEnforcestartdatevalidity(defaults.getDomain().isEnforceStartDateValidity());
            d.setEnforcestartdatevalidityinherited(false);
            d.setEnforceenddatevalidity(defaults.getDomain().isEnforceEndDateValidity());
            d.setEnforceenddatevalidityinherited(false);
            d.setMultiplepsnallowed(defaults.getDomain().isAllowMultiplePsn());
            ...
            algorithm.setSalt(DomainController.generateSalt(defaults.getAlgorithm().getSaltLength()));
