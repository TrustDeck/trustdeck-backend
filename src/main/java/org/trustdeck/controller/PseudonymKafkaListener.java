...
public class PseudonymKafkaListener {
	...
	public void onMessage(Message<String> message, Acknowledgment ack) {
        ...
        if (domain == null) {
            ...
            algorithm.setName(DomainController.DEFAULT_PSEUDONYMIZATION_ALGO);
            algorithm.setAlphabet(DomainController.DEFAULT_PSEUDONYMIZATION_ALPHABET);
            algorithm.setRandomAlgorithmDesiredSize(DomainController.DEFAULT_RANDOM_ALGORITHM_DESIRED_SIZE);
            algorithm.setRandomAlgorithmDesiredSuccessProbability(DomainController.DEFAULT_RANDOM_ALGORITHM_DESIRED_SUCCESS_PROBABILITY);
            algorithm.setConsecutiveValueCounter(1L);
            algorithm.setPseudonymLength(DomainController.DEFAULT_PSEUDONYM_LENGTH);
            algorithm.setPaddingCharacter(String.valueOf(DomainController.DEFAULT_PADDING_CHARACTER));
            algorithm.setAddCheckDigit(DomainController.DEFAULT_ADD_CHECK_DIGIT);
            algorithm.setLengthIncludesCheckDigit(DomainController.DEFAULT_LENGTH_INCLUDES_CHECK_DIGIT);
            algorithm.setSalt(DomainController.generateSalt(DomainController.DEFAULT_SALT_LENGTH));
            algorithm.setSaltLength(DomainController.DEFAULT_SALT_LENGTH);
            d.setAlgorithmId(algorithmDBService.createOrGetAlgorithm(algorithm.convertToPOJO()));
        ...
        if (algorithmDBService.getAlgorithmByID(domain.getAlgorithmId()).getName().toUpperCase().startsWith("RANDOM")) {
